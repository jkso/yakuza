...
var sumProperties = function (current, next) {
  ...
  if (_.isObject(current)) {
    _.each(_.keys(current), function (key) {
      ...
      newObjext[key] = currentValue + nextValue;
    });
  } else {
    newObject = next;
  }
...
// Define a global sharing method. `current` refers to the actual value of the key in which we
// are sharing. `Next` is the value we will now share in the key
...
Yakuza.addShareMethod('sumProperties', sumProperties);

Yakuza.task('TestScraper', 'TestAgent', 'JoinStrings')
